...
# idenLib_appdata = %LOCALAPPDATA%\idenLib
# idenLibCache    = %LOCALAPPDATA%\idenLib\idenLibCache64
# idenLibCacheMain= %LOCALAPPDATA%\idenLib\idenLibCacheMain64
# symEx_dir       = {IDAPRO OATH}\SymEx\
func_sigs = {}
mainSigs = {}

ida_dir = ida_diskio.idadir("")
symEx_dir = ida_dir + os.sep + "SymEx"
print(f"symEx_dir={symEx_dir}")
...
def getOpcodes(addr, size):
    ...
    opcodes_buf = bytearray()
    for i in md.disasm(instr_bytes, size):
        # get last opcode
        if (i.opcode[3] != 0):
            opcodes_buf.append(i.opcode[3])
        elif (i.opcode[2] != 0):
            opcodes_buf.append(i.opcode[2])
        elif(i.opcode[1] != 0):
            opcodes_buf.append(i.opcode[1])
        else:
            opcodes_buf.append(i.opcode[0])
    return opcodes_buf.hex()
...
def idenLibProcessSignatures():
    ...
    for file in getFiles(symEx_dir):
        ...
        with open(file, 'rb') as ifile:
            ...
            sig = sig.split(b"\n")
            for line in sig:
                sig_opcodes, name = line.decode("utf-8").split(" ")
...
def idenLib():
    ...
    for sig_opcodes, addr in func_bytes_addr.items():
        if func_sigs.__contains__(sig_opcodes):
            ...
            if (current_name != func_name):
                ...
                counter = counter + 1
        if mainSigs.__contains__(sig_opcodes): # "main" sig
